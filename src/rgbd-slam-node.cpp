...
void setup_ros_publishers(rclcpp::Node &node) {
    ...
    map_points_pub = node.create_publisher<sensor_msgs::msg::PointCloud2>("/points2", 1000);
    ...
    m_SLAM(pSLAM)
{
    ...
    depth_sub = std::make_shared<message_filters::Subscriber<ImageMsg> >(shared_ptr<rclcpp::Node>(this), "/Depth");
    //depth_sub = std::make_shared<message_filters::Subscriber<ImageMsg> >(shared_ptr<rclcpp::Node>(this), "/camera/depth/image_raw");
    
    syncApproximate = std::make_shared<message_filters::Synchronizer<approximate_sync_policy>>(approximate_sync_policy(640), *rgb_sub, *depth_sub);
